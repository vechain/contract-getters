...
export const getXApps = async (
    ...
    const unendorsedIds = new Set(unendorsedApps.map((app: any) => app.id));

    return {
        allApps: Object.values(allApps),
        active: apps.map((app: any) => ({
            ...app,
            createdAtTimestamp: app.createdAtTimestamp.toString(),
        })),
        unendorsed: unendorsedApps.map((app: any) => ({
            ...app,
            createdAtTimestamp: app.createdAtTimestamp.toString(),
        })),
        endorsed: apps
            .filter((app: any) => !unendorsedIds.has(app.id))
            .map((app: any) => ({
                ...app,
                createdAtTimestamp: app.createdAtTimestamp.toString(),
            })),
