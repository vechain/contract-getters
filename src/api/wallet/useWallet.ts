...
import { useGetChainId } from '@thor/blocks'
import { useGetNodeUrl } from '@utils'
import { useWallet as useDAppKitWallet } from '@vechain/dapp-kit-react'
import { useSmartAccountVersion } from '@thor/smartAccounts'
import { useCrossAppConnectionCache } from '@cache';
import { useSmartAccount } from '@thor/smartAccounts';
...
export const useWallet = (config: WalletConfig): UseWalletReturnType => {
    const {
    ...
    const isConnectedWithVeChain =
        (isConnectedWithSocialLogin && config.privyAppId === process.env.VECHAIN_PRIVY_APP_ID) ||
        (isConnectedWithCrossApp &&
            connectionCache?.ecosystemApp?.appId === process.env.VECHAIN_PRIVY_APP_ID);
    ...
    return {
        ...
        connection: {
            ...
            nodeUrl,
            delegatorUrl: config.delegatorUrl,
            chainId: chainId,
