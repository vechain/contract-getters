 ...
 */
export const useGetAvatarLegacy = (
    networkType: NETWORK_TYPE,
    name: string
) => {
    const nodeUrl = getConfig(networkType).nodeUrl;

    const avatarQuery = useQuery({
        queryKey: getAvatarLegacyQueryKey(name ?? '', networkType),
        queryFn: async () => {
            if (!name) return null;

            return getAvatarLegacy(networkType, nodeUrl, name);
        },
        enabled: !!name && !!nodeUrl && !!networkType,
    });
