...
const getNetworkType = () => {
    return isBrowser
        ? localStorage.getItem(VECHAIN_KIT_STORAGE_KEYS.NETWORK)
        : process.env.NETWORK_TYPE || 'solo'
};

export const ENV = {
    get isDevelopment() {
        return getNetworkType() === 'test' || getNetworkType() === 'solo';
    },
    get isProduction() {
        return getNetworkType() === 'main';
    }
};
